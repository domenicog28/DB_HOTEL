...
cursore.execute("create table prenotazioni (id_prenotazione int primary key auto_increment, cliente int not null references clienti(id_cliente),n_ospiti int not null, ch_in date not null, ch_out date not null, stanza int not null references camere(n_camera))engine=innodb")
...
cursore.execute("create table prodotti (cod_prodotto char(8) primary key, nome varchar(255) not null, fornitore char(11) not null references fornitori(p_iva))engine=innodb;")

cursore.execute("create table ordini (id_ordine int primary key auto_increment, stato enum('a','i','r','p') not null)engine=innodb;")

cursore.execute("create table lista_ordine(id int primary key auto_increment, ordine int not null references ordini(id_ordine), prodotto char(8) not null references prodotti(cod_prodotto), quantità int not null)engine=innodb;")
...
cursore.execute("create table ricevuti (ordine int references ordini(id_ordine),fornitore char(11) not null references fornitori(p_iva), data_ricezione date not null, stato_pagamento enum('effettuato','non effettuato') not null, primary key(ordine))engine=innodb;")
...
cursore.execute("create index ch_in_out on prenotazioni (ch_in,ch_out);")

cursore.execute("create index forn on prodotti(fornitore);")

cursore.execute("create index p_eff on effettuati(data);")

cursore.execute("create index p_data on ricevuti(data_ricezione);")

cursore.execute("create index p_d_p on pagamenti(data_pagamento);")
...
cursore.executemany(in_ric,ric)
